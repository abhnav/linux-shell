...
#define execerrorstatus 10
#define INVALINPUT -5
#define EXECERROR -7
#define SUCCESS 1
#define ERRPIPECOPY -10
#define WRITEERROR -9
...
int* Open(cmd_t *command){
    ...
    for(;i<n;i++){
        if(command->append[i] == 1){
            ...
            printf("fd %d opened for append from %s\n",fds[i],command->outfile[i]);
        }
        else{
            fds[i] = open(command->outfile[i],O_WRONLY|O_CREAT,0664);
            if(fds[i]<0)
                return NULL;
            printf("fd %d opened for write from %s\n",fds[i],command->outfile[i]);
        }
    }
    return fds;
}
...
void sighand_int(int signo){
    printf("last ten commands(from newest to oldest) executed by user:\n");
    caughtint = 1;
}
    ...
    for(int i = 0;i<numberOfCommands;i++){
        ...
        if(ttl <= 0){//this means that either you are starting or you are the command just before a pipe
            ...
            if(cmdTokenPipeDepth[i]>0){
                fprintf(stderr,"you are piping to less commands than specified\n");
            ...
            int coppipe = copyPipe(in,ptemp1,ptemp2);
            if(coppipe == -1){
                prev[pt].status = ERRPIPECOPY;
                pt = incre(pt);
                restart = 1;
                break;
            }
        ...
        if(commands[i].numinfl){
            ...
            printf("fd %d opened for input from %s\n",in,commands[i].infile[commands[i].numinfl-1]);
        }
        int forknum;
        if(!(forknum = fork())){
            close(pp[0]);
            ...
            _exit(execerrorstatus);
        }
        close(pp[1]);
        /*if(in != STDIN_FILENO)*/
            /*close(in);*/
        int a,tnum;
        printf("child process with pid %d opened to execute %s\n",forknum,commands[i].cmdargs[0]);
        tnum = waitpid(-1,&status,0);//will see what to do with status
        if(tnum <-1)
            if(errno == EINTR)
                continue;
        if(WIFEXITED(status)){
            tnum = WEXITSTATUS(status);
            if(tnum == 10){
